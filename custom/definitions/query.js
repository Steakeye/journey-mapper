...
function scripts_nodeMatchesSelector() {
    ...
    var docEl = document.documentElement, matchesSelector = docEl.webkitMatchesSelector || docEl.mozMatchesSelector || docEl.msMatchesSelector || docEl.oMatchesSelector || docEl.matchesSelector;
...
function scripts_nodeFunctionCall() {
    var el = arguments[0], name = arguments[1], args = Array.prototype.slice.call(arguments, 2);
...
function scripts_nodeSelectOption() {
    ...
    function scripts_nodeTrigger() {
        var el = arguments[0], type = arguments[1], data = arguments[2];
    ...
    if (opt == null)
        opt = find(byAttr("id"));
    if (opt == null)
        throw Error("Option not found: " + str);
    var optEl = opt[0], index = opt[1];
...
function scripts_nodeClassAdd() {
    // source ./_inline.es6
    "use strict";
    var el = arguments[0], klass = arguments[1];
...
(function () {
    ...
    _.obj_extend(SQueryProto, {
        ...
        type: function type(str) {
            var arr = toSequance(str), fn = getSequenceFunction(arr);
            return async_each(this, function ($, node) {
